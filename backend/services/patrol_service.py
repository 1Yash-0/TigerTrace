...
def _calculate_station_metrics(
    ...
    # Village-adjacency is data-driven (Capture.zone), not a hard-coded station list
    is_village = any(c.zone == "village_adjacent" for c in captures)
    ...
    elif zone in ("survey", "unclassified", None):
        # The PTR workbook supplies camera coordinates but no core/buffer
        # boundary.  Keep this neutral instead of presenting an unsupported
        # conflict-risk classification as fact.
        conflict_score = 20 + min(20, len(tiger_ids) * 2)
        conflict_evidence.append("PTR survey station has no source core/buffer classification")
    else:
        # Explicit core zone
...
def get_station_patrol_priorities(db: Session) -> list[dict]:
    ...
    if not all_station_ids:
        return []  # no captures in DB — no patrol data to compute (truthful empty)
...
def get_station_patrol_detail(station_id: str, db: Session) -> Optional[dict]:
    ...
    """
    # Normalize station ID — real PTR camera GRID ids ("C090"); legacy "ST-xx"
    # ids pass through untouched for old records.
    sid = station_id.upper().strip()
    m = re.match(r"^C-?(\d{1,3})$", sid)
    if m:
        sid = f"C{int(m.group(1)):03d}"
    elif not sid.startswith("ST-"):
        num_str = "".join(filter(str.isdigit, sid))
        if num_str:
            sid = f"C{int(num_str):03d}"
