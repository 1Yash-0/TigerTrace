...
def get_village_adjacent_stations(db: Session) -> set:
    """
    Stations classified as village-adjacent, derived strictly from capture data
    (Capture.zone == "village_adjacent"). The legacy demo hard-coded a fixed
    ST-xx set; with the real PTR GRID dataset (C0xx stations + GPS coords)
    village-adjacency is only asserted when the data says so. Returns an empty
    set until zone data marks stations as village-adjacent.
    """
    rows = db.query(Capture.station_id).filter(Capture.zone == "village_adjacent").distinct().all()
    ...
    for tiger in tigers:
        ...
        for capture in recent:
            if capture.station_id not in historical_stations:
                # Village-adjacency is data-driven (Capture.zone), not a fixed list
                is_village = capture.zone == "village_adjacent"
