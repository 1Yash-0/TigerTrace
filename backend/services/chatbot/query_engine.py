...
from services.alert_service import get_village_adjacent_stations
...
def _get_station_activity(entities: dict, db: Session) -> dict:
    ...
    if station_id:
        ...
        return {
            ...
            "is_village_adjacent": any(c.zone == "village_adjacent" for c in captures),
        }
    ...
    village_stations = get_village_adjacent_stations(db)
    result = []
    for sid, count in station_counts:
        ...
        result.append({
            ...
            "is_village_adjacent": sid in village_stations,
        })
...
def _get_high_risk_stations(entities: dict, db: Session) -> dict:
    ...
    result = []
    village_stations = get_village_adjacent_stations(db)
    for sid in village_stations | alert_stations:
        ...
        result.append({
            ...
            "risk_level": "high" if sid in village_stations and alert_count > 0 else
                          "medium" if sid in village_stations or alert_count > 0 else "low",
        })
...
def _get_village_proximity(entities: dict, db: Session) -> dict:
    result = []
    for sid in get_village_adjacent_stations(db):
...
def _get_patrol_trend(entities: dict, db: Session) -> dict:
    ...
    if not station_id:
        top = get_station_patrol_priorities(db)
        if not top:
            return {"error": "No station patrol data available yet."}
        station_id = top[0]["station_id"]
