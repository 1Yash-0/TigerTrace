...
class MDV6Detector:
    ...
    def detect_image(self, image_path, conf_thresh=0.20, iou_thresh=0.45):
        ...
        # Per-class NMS — the YOLO export returns heavily overlapping
        # candidates, so without suppression one physical animal yields many
        # boxes (each would become a duplicate crop downstream). Per-class
        # keeps e.g. a person box from being suppressed by an animal box.
        if detections:
            keep_all = []
            for cid in {d["class_id"] for d in detections}:
                idxs = [i for i, d in enumerate(detections) if d["class_id"] == cid]
                boxes_xywh = [
                    [detections[i]["bbox"][0], detections[i]["bbox"][1],
                     detections[i]["bbox"][2] - detections[i]["bbox"][0],
                     detections[i]["bbox"][3] - detections[i]["bbox"][1]]
                    for i in idxs
                ]
                scores = [detections[i]["confidence"] for i in idxs]
                keep = cv2.dnn.NMSBoxes(boxes_xywh, scores, float(conf_thresh), float(iou_thresh))
                if keep is not None and len(keep) > 0:
                    keep_all.extend(idxs[int(k)] for k in np.array(keep).flatten())
            detections = [detections[i] for i in sorted(keep_all)]
