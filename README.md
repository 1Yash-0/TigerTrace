...
# Download the required model weights (MDV6-yolov9-c detector, ~101MB, md5-verified)
...
started.  To reconcile new PTR workbook/images into the existing database
without retraining models, run:

```bash
python import_real_data.py
```

The importer is idempotent; `--fresh` is reserved for an intentional full
database rebuild.  Model training and weight generation are separate manual
operations and are never part of startup or data reconciliation.
