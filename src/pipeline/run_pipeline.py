...
4. Stripe Re-ID Embedding & Persistent Centroid-Based Gallery Retrieval (Swin-Transformer ArcFace side-aware)
...
# Allow direct execution (`python src/pipeline/run_pipeline.py`) by putting the
# repository root on sys.path before the src.* imports below.
...
def run_end_to_end_pipeline(input_folder, run_id="pench_run_001", device="cuda"):
    ...
    for item in tqdm(animal_detections, desc="Species Classification"):
        ...
        try:
            ...
            # A crop that cannot be classified is NOT silently passed through as
            # a tiger — it is skipped and reported.
            cls_error_count += 1
            print(f"[WARN] Species classification failed for {item['detection_id']}: {e} — crop skipped")

    print(f"Species Gate: {len(tiger_detections)} Tigers confirmed | {non_tiger_count} Non-tigers filtered out | {cls_error_count} Classification errors skipped.")

    # ── 4. Persistent Centroid-Based Tiger Re-ID Matching ────────────────────
    print("\n[STAGE 4/6] Tiger Flank Stripe Re-ID Matching (Persistent Side-Aware Gallery)...")
    reid_cfg = cfg["reid"]
    reid_onnx_path = reid_cfg.get("onnx_path", "models/tigertrace_ptr_v2_side_aware/tigertrace_ptr_v2_side_aware.onnx")
    emb_dim = int(reid_cfg.get("embedding_dim", 512))
    dist_auto = float(reid_cfg.get("auto_match_distance_threshold", 0.30))
    margin_min = float(reid_cfg.get("ambiguity_margin_threshold", 0.05))
    qual_min = float(reid_cfg.get("min_quality_score_for_automatch", 0.50))

    from src.reid.extractor import TigerReIDExtractor
    reid_extractor = TigerReIDExtractor(onnx_model_path=reid_onnx_path)
    print(f"Loaded Production Re-ID Extractor: {reid_extractor.model_path}")

    # Initialize Persistent Gallery
    gallery = PersistentGallery(dim=emb_dim)
    gallery.load()
    ...
    for item in tqdm(tiger_detections, desc="Tiger Re-ID"):
        ...
        try:
            ...
            res = reid_extractor.extract(crop_im)
            q_feat = np.array(res["embedding"], dtype=np.float32)
            q_flank = res["flank_name"]
            ...
            if decision == "AUTO_ENROLL_NEW":
                ...
                gallery.enroll(assigned_id, q_feat, flank=q_flank)
