...
def scan_input_directory(input_dir, output_inventory_path):
    ...
    # Case-insensitive extension match with de-duplication: on Windows,
    # glob.glob("*.jpg") and glob.glob("*.JPG") both match every file, which
    # used to double-inventory every frame on the card.
